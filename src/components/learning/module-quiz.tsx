...
export function ModuleQuiz({ moduleId, pathId, quizzes, nextModuleId }: ModuleQuizProps) {
  ...
  const submitAll = async () => {
    ...
    // Compute score and pass status HERE before setting submitted state
    const computedScore = answers.reduce((acc, a, i) => (a === quizzes[i].correct_index ? acc + 1 : acc), 0)
    const computedPassed = computedScore >= Math.ceil(quizzes.length * 0.6)

    console.log('[ModuleQuiz] submitAll:', { computedScore, computedPassed, total: quizzes.length, answers })
      ...
      const { error, data } = await supabase.from('user_module_progress').upsert(
        {
          user_id: user.id,
          module_id: moduleId,
          status: computedPassed ? 'completed' : 'in_progress',
          quiz_score: computedScore,
          quiz_attempts: 1,
          completed_at: computedPassed ? new Date().toISOString() : null,
          updated_at: new Date().toISOString(),
        },
        { onConflict: 'user_id,module_id' },
      ).select().single()

      console.log('[ModuleQuiz] upsert result:', { error, data })

      if (error) throw error
      setSubmitted(true)
      toast({
        title: computedPassed ? 'Quiz passed!' : 'Keep studying',
        description: `${computedScore} / ${quizzes.length} correct`,
      })
      router.refresh()

      if (computedPassed && nextModuleId) {
        router.push(`/learn/${pathId}/modules/${nextModuleId}`)
      }
    } catch (e: unknown) {
      console.error('[ModuleQuiz] upsert error:', e)
      toast({
