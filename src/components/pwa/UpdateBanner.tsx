...
function hasUnsavedFormData(): boolean {
  ...
  const handleUpdate = useCallback(() => {
    if (hasUnsavedFormData()) {
      const confirmed = window.confirm(
        'An update is ready. You have unsaved changes that will be lost. Update now?'
      )
      if (!confirmed) return
    }
    ...
    <div className="fixed top-0 left-0 right-0 z-50 p-3 bg-blue-600 text-white border-b shadow-lg animate-in slide-in-from-top-5">
      <div className="container flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <Button size="sm" variant="ghost" onClick={handleDismiss} className="text-white hover:text-white/80">
            <X className="h-4 w-4" />
          </Button>
        </div>
      </div>
