...
export async function POST(request: Request) {
  try {
    ...
    const speakerName = (() => {
      if (!sermon.speaker) return ''
      if (Array.isArray(sermon.speaker)) return sermon.speaker[0]?.name || ''
      return (sermon.speaker as any).name || ''
    })()
    const seriesTitle = (() => {
      if (!sermon.series) return ''
      if (Array.isArray(sermon.series)) return sermon.series[0]?.title_en || ''
      return (sermon.series as any).title_en || ''
    })()
